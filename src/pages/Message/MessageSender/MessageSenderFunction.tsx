...
async function validate(form: Partial<MessageTypes> & Partial<ProcessTypes>, setErrorMessage: CallableFunction) {
    setErrorMessage('');
    let error = false;
...
export async function handleForm(e: FormEvent, form: Partial<MessageTypes> & Partial<ProcessTypes>, navigate: CallableFunction, setErrorMessage: CallableFunction): Promise<void> {
    e.preventDefault();
...
export async function handleUsers(user: UserTypes): Promise<UserTypes[] | null> {
    const users = await siscopIndex('users', 0, 0, 0, { select: '-password' });
    const { response }: { response: UserTypes[] | null } = users.data;
...
export async function handleSections(): Promise<SectionTypes[] | null> {
    ...
    const { response }: { response: SectionTypes[] | null } = sections.data;
    const sectionsArray = response ? response.filter((element) => element.level !== 10) : null;
    return sectionsArray;
}

export async function handleProcesses(user: UserTypes, processId?: string | null): Promise<ProcessTypes[] | null> {
    if (processId) {
        const process = await siscopShow('processes/process', 0, { _id: processId });
        const { response }: { response: ProcessTypes } = process.data;
    ...
    const done: ProcessTypes[] | null = processesDone.data.response;
    const received: ProcessTypes[] | null = processesReceived.data.response;
    let processArray: ProcessTypes[] | null = null;
    if (done !== null) processArray = [...done];
    if (received !== null) processArray = processArray ? [...processArray, ...received] : [...received];
    return processArray;
}

export function generateUserSelect(users: UserTypes[] | null, setForm: CallableFunction): ReactNode {
    if (users) {
        setForm((current: Partial<MessageTypes>) => ({ ...current, section_receiver: undefined }));
...
export function generateSectionSelect(section: SectionTypes[] | null, setForm: CallableFunction): ReactNode {
    if (section) {
        setForm((current: Partial<MessageTypes>) => ({ ...current, receiver: undefined }));
        const handleInput = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setInputs(e, setForm);
...
export function generateProcessSelect(process: ProcessTypes[] | null, processId: string | null, setForm: CallableFunction): ReactNode {
    if (process) {
        const handleInput = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setInputs(e, setForm);
        if (processId) {
