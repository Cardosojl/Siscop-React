...
function DocumentList({ path, dispatchUser, user }: SimpleView): JSX.Element {
    ...
    useEffect(() => {
        handleProcess(path as string, user, processId).then((data) => {
            setProcess(data);
        }).catch((error) => {
    ...
    return (
        <div className="MainWindow">
            <div className="Window">
                ...
                <hr />
                <div className="DocumentList__table"><Table table={table} /></div>
                <ProcessInformation process={ process } />
                <UploadFiles setRefresh={setRefresh}/>
                <div className="DocumentList__buttons__fild">
                    <button className="Button--green DocumentList__button" onClick={() => navigate(-1)}><img className="DocumentList__arrow" src={arrow} /></button>
                    <button className="Button--blue DocumentList__button"><Link to={`/novaMensagem/${processId}`}><img className="DocumentList__mail" src={mail} /></Link></button>
                </div>
            </div>
        </div>
    );
